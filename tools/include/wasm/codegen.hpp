...
#include <wasm/gen.hpp>

#include <wasm/utils.hpp>
#include <wasm/whereami/whereami.hpp>
#include <wasm/abi.hpp>
...
namespace wasmcdt { namespace cdt {
   ...
   struct codegen_exception : public std::exception {
      virtual const char* what() const throw() {
         return "wasm.codegen fatal error";
      }
   };
   ...
   // remove after v1.7.0
   bool has_wasmlib = false;

   class wasm_ppcallbacks : public PPCallbacks {
      public:
         wasm_ppcallbacks(SourceManager& sm, std::string file) : sources(sm), fn(file) {}
      protected:
         virtual void InclusionDirective(
            ...
            if ( file_name.find("wasmlib") != StringRef::npos )
               has_wasmlib = true;
         }
   ...
   class wasm_codegen_visitor : public RecursiveASTVisitor<wasm_codegen_visitor>, public generation_utils {
      private:
      ...
      public:
         ...
         explicit wasm_codegen_visitor(CompilerInstance *CI)
         ...
         bool is_datastream(const QualType& qt) {
            ...
            auto ds_re    = std::regex("(((class wasm::)?datastream<[a-zA-Z]+[a-zA-Z0-9]*.*>)|(DataStream)) &");
         ...
         */
         ...
         void create_dispatch(const std::string& attr, const std::string& func_name, F&& get_str, CXXMethodDecl* decl) {
            ...
            if (cg.is_wasm_contract(decl, cg.contract_name)) {
               ...
               ss << "}\n";
               ss << "wasm::datastream<const char*> ds{(char*)buff, as};\n";
               int i=0;
         ...
         void create_action_dispatch(CXXMethodDecl* decl) {
            ...
            create_dispatch("wasm_wasm_action", "__wasm_action_", func, decl);
         }

         void create_notify_dispatch(CXXMethodDecl* decl) {
            auto func = [](CXXMethodDecl* d) { return generation_utils::get_notify_pair(d); };
            create_dispatch("wasm_wasm_notify", "__wasm_notify_", func, decl);
         }
         ...
         virtual bool VisitCXXMethodDecl(CXXMethodDecl* decl) {
            ...
            if (decl->isWasmAction()) {
               name = generation_utils::get_action_name(decl);
               validate_name(name, [&]() {emitError(*ci, decl->getLocation(), "action not a valid wasm name");});
      ...
      class wasm_codegen_consumer : public ASTConsumer {
      private:
         wasm_codegen_visitor *visitor;
         std::string main_file;
         CompilerInstance* ci;

      public:
         explicit wasm_codegen_consumer(CompilerInstance *CI, std::string file)
            : visitor(new wasm_codegen_visitor(CI)), main_file(file), ci(CI) { }
         ...
         virtual void HandleTranslationUnit(ASTContext &Context) {
            ...
            if (main_fe &&
               ...
               try {
                  ...
                  std::stringstream ss;
                  ss << "extern \"C\" {\n";
                  ss << "void wasm_assert_code(uint32_t, uint64_t);";
                  ss << "\t__attribute__((weak, wasm_wasm_entry, wasm_wasm_abi(";
                  std::string abi = cg.abi;
                  ss << "\"" << _quoted(abi) << "\"";
                  ss << ")))\n";
                  ss << "\tvoid __insert_wasm_abi(unsigned long long r, unsigned long long c, unsigned long long a){";
                  ss << "wasm_assert_code(false, 1);";
                  ss << "}\n";
                  ss << "}";
      ...
      class wasm_codegen_frontend_action : public ASTFrontendAction {
      public:
         virtual std::unique_ptr<ASTConsumer> CreateASTConsumer(CompilerInstance &CI, StringRef file) {
            CI.getPreprocessor().addPPCallbacks(_make_unique<wasm_ppcallbacks>(CI.getSourceManager(), file.str()));
            return _make_unique<wasm_codegen_consumer>(&CI, file);
         }
   };
