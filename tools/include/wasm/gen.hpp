...
struct generation_utils {
      ...
      auto check = [&](const clang::Type* pt) {
        if (auto tst = llvm::dyn_cast<clang::TemplateSpecializationType>(pt))
         if (auto rt = llvm::dyn_cast<clang::RecordType>(tst->desugar()))
            return rt->getDecl()->isWasmIgnore();
      ...
      auto get = [&](const clang::Type* pt) {
         if (auto tst = llvm::dyn_cast<clang::TemplateSpecializationType>(pt))
            if (auto decl = llvm::dyn_cast<clang::RecordType>(tst->desugar()))
               return decl->getDecl()->isWasmIgnore() ? tst->getArg(0).getAsType() : type;
         return type;
      };
   ...
   static inline bool has_wasm_ricardian( const clang::CXXMethodDecl* decl ) {
      return decl->hasWasmRicardian();
   }
   static inline bool has_wasm_ricardian( const clang::CXXRecordDecl* decl ) {
      return decl->hasWasmRicardian();
   }

   static inline std::string get_wasm_ricardian( const clang::CXXMethodDecl* decl ) {
      return decl->getWasmRicardianAttr()->getName();
   }
   static inline std::string get_wasm_ricardian( const clang::CXXRecordDecl* decl ) {
      return decl->getWasmRicardianAttr()->getName();
   }

   static inline std::string get_action_name( const clang::CXXMethodDecl* decl ) {
      std::string action_name = "";
      auto tmp = decl->getWasmActionAttr()->getName();
      if (!tmp.empty())
         return tmp;
   ...
   static inline std::string get_notify_pair( const clang::CXXMethodDecl* decl ) {
      ...
      auto tmp = decl->getWasmNotifyAttr()->getName();
      return tmp;
   }
   ...
   static inline bool is_wasm_contract( const clang::CXXMethodDecl* decl, const std::string& cn ) {
      std::string name = "";
      if (decl->isWasmContract())
         name = decl->getWasmContractAttr()->getName();
      else if (decl->getParent()->isWasmContract())
         name = decl->getParent()->getWasmContractAttr()->getName();
      if (name.empty()) {
   ...
   static inline bool is_wasm_contract( const clang::CXXRecordDecl* decl, const std::string& cn ) {
      std::string name = "";
      auto pd = llvm::dyn_cast<clang::CXXRecordDecl>(decl->getParent());
      if (decl->isWasmContract()) {
         auto nm = decl->getWasmContractAttr()->getName().str();
         name = nm.empty() ? decl->getName().str() : nm;
      }
      else if (pd && pd->isWasmContract()) {
         auto nm = pd->getWasmContractAttr()->getName().str();
