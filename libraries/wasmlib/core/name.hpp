...
namespace wasm {
   ...
   public:
      ...
      {
         if( str.size() > 13 ) {
            wasm::check( false, "string is too long to be a valid name" );
         }
         if( str.empty() ) {
            return;
         }
         ...
         if( str.size() == 13 ) {
            ...
            if( v > 0x0Full ) {
               wasm::check(false, "thirteenth character in name cannot be a letter that comes after j");
            }
            value |= v;
         }
      }
...
inline constexpr wasm::name operator""_n() {
   constexpr auto x = wasm::name{std::string_view{wasm::detail::to_const_char_arr<Str...>::value, sizeof...(Str)}};
   return x;
}
